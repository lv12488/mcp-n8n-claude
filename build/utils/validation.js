...
exports.transformConnectionsToArray = transformConnectionsToArray;
...
function validateWorkflowSpec(input) {
    if (!input || typeof input !== 'object') {
        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, 'Workflow spec must be an object');
    }
    if (!Array.isArray(input.nodes)) {
        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, 'Workflow nodes must be an array');
    }
    if (!Array.isArray(input.connections)) {
        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, 'Workflow connections must be an array');
    }
    if (input.connections.length === 0) {
        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, 'Workflow connections array cannot be empty. Nodes must be connected.');
    }
    ...
    const formattedNodes = (input.nodes || []).map((node, index) => {
        if (typeof node !== 'object' || typeof node.type !== 'string' || typeof node.name !== 'string') {
            throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, 'Each node must have a type and name');
        }
        ...
        if (node.type === 'n8n-nodes-base.set' && node.parameters && node.parameters.values) {
            ...
            // Handle case when values is already an array (legacy structure)
                ...
                formattedValues = node.parameters.values.map((value) => {
                    return {
                        name: value.name,
                        value: value.value,
    ...
    if (input.connections && Array.isArray(input.connections)) {
        ...
        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, 'Workflow connections are missing or invalid. Please provide a valid connections array.');
    }
 ...
 */
function transformConnectionsToArray(connections) {
    ...
    Object.entries(connections).forEach(([sourceName, sourceData]) => {
        ...
        sourceData.main.forEach((outputConnections, sourceOutput) => {
